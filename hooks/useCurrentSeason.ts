...
export function useCurrentSeason(pollMs = 30_000) {
  ...
  const label = season?.name || (season?.season_number != null ? `${leagueConfig.shortName} ${season.season_number}` : null);
  const displayName = label || leagueConfig.copyPrefixes.marketing;

  return { season, ready, label, displayName, leagueConfig };
}
