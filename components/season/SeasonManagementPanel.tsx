...
export function SeasonManagementPanel() {
  ...
  const activeLeagueConfig = useMemo(() => getLeagueConfig(current?.league_code), [current]);
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-white/55">
            End current season safely, start next {activeLeagueConfig.shortName} season, and import old data without deleting anything.
          </p>
          ...
          <select className="input" value={sourceSeasonId} onChange={(event) => setSourceSeasonId(event.target.value)}>
            ...
            {oldSeasons.map((season) => (
              <option value={season.id} key={season.id}>
                [{season.league_code || 'APL'}] {season.name}
              </option>
            ))}
          </select>
