...
const getFaqs = (config: LeagueConfig | null): FaqItem[] => {
  const shortName = config ? config.shortName : 'APL';
  
  return [
  {
    question: `What are the team budget and squad size rules in ${shortName}?`,
    answer:
      'Each franchise team operates with a maximum budget of ₹50,000. Each complete squad consists of 4 bought Players + 1 Captain.',
  },
  {
    question: `How do player registrations and approvals work in ${shortName}?`,
    answer:
...
export function FaqSection() {
  ...
  const { leagueConfig } = useCurrentSeason();
  const faqs = getFaqs(leagueConfig);
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6">
        ...
        <div className="text-center space-y-4 mb-16">
          ...
          <p className="text-slate-400 text-base md:text-lg">
            Everything you need to know about the {leagueConfig ? leagueConfig.copyPrefixes.marketing : 'APL Digital Auction'} platform.
          </p>
        </div>
